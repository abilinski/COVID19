...
server <- function(input, output, session) {
  ...
  Re_td_value_exp_int <- reactive({
    ...
    avg_s_t = mean(s_t[start_t:end_t,2])/param_vec$n
    # Edit this to compute Re and td from exponential curve (done)
    Re = round(as.numeric(calc_Re_td_from_exp(param_vec_int, cum_case, avg_s_t)[2]), digits=3)
    td = round(as.numeric(calc_Re_td_from_exp(param_vec_int, cum_case, avg_s_t)[1]), digits=3)
    return (c(Re, td))
  })
